        ...
        success: function (result) {
            ...
            $.each(result, function (i, jobs) {
                ...
                $.each(jobs, function (j, a) {
                    ...
                    list += '<span class="badge">' + (a.time_long / 3600).toFixed(1) + ' </span > ';
                    list += '</li>\n';
                });

                var day_job_time = '<span class="day_job_time">' + (rest / 3600).toFixed(1) + '/' + (total / 3600).toFixed(1) + '</span>';
...
function init_todo_list() {
    //确定当前周数，用于切换前后周
    $('#todo-week').attr('week_date', new Date().Format('yyyy-MM-dd'));
