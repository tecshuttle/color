...
class questionanswer extends MY_Controller
{
    ...
    {
        $data['bases_item'] = $this->questionanswer_model->get_bases($slug);
        ...
        $this->load->view('questionanswer/view', $data);
    ...
    public function region()
    {
        ...
        if ($province_id)
        {
            $rows = $this->questionanswer_model->get_regions(2, $province_id);
            if ($rows)
            {
                foreach ($rows as $row)
                {
    ...
    public function save()
    {
        ...
        if (isset($_POST['id'])) {
            $_POST['mtime'] = time();
            $this->questionanswer_model->update($_POST);
        } else {
            $_POST['ctime'] = time();
            $_POST['mtime'] = time();
            $this->questionanswer_model->insert($_POST);
        }
    ...
    {
        $option = $_POST;
        $data = $this->questionanswer_model->get($option);
    ...
    public function delete()
    {
        ...
        $this->questionanswer_model->deleteByID($id);
